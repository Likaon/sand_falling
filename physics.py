...
class PhysicsEngine:
    ...
    def criar_grao(self, pos: tuple[int, int]) -> pymunk.Shape | None:
        ...
        if len(self.areia_shapes) >= config.MAX_PARTICULAS:
            ...
            return None
        body = pymunk.Body(mass=config.MASSA_GRAO,
                           moment=pymunk.moment_for_circle(config.MASSA_GRAO, 0, config.RAIO_GRAO))
        body.position = pos
    ...
    def criar_segmento(self, p0: tuple[int, int], p1: tuple[int, int], espessura: int = 2) -> None:
        """
    ...
    def criar_bordas(self) -> None:
        ...
        self.criar_segmento((0, self.altura - 1), (self.largura, self.altura - 1), 1)
        self.criar_segmento((0, 0), (0, self.altura), 1)
        self.criar_segmento((self.largura - 1, 0), (self.largura - 1, self.altura), 1)
    ...
    def limpar_fora_da_tela(self, largura: int, altura: int) -> None:
        ...
        for s in self.areia_shapes:
            ...
            if -50 <= x <= largura + 50 and -50 <= y <= altura + 200:
                ...
                self.space.remove(s, s.body)
                logging.info(f"Grão removido fora da tela em {(x, y)}")
